...
export const Home = () => {
  ...
  const [query, setQuery] = useState<string>("");
  const [state, setState] = useState<IRECIPERESPONSE[]>(data || []); // Start with empty array or initial data

  // Handle error in fetching recipes
  useEffect(() => {
    if (error) {
      console.error(error);
      cogoToast.error("Failed to load recipes. Please try again.");
    }
  }, [error]);
    ...
    <Suspense fallback={<UILoader />}>
      <div className="text-white w-full h-full">
        ...
        {loading ? (
          ...
          <div className="flex flex-wrap gap-3 flex-col items-center justify-center md:justify-start md:items-start md:flex-row w-full">
            {!!state.length ? (
              state.map((recipe: IRECIPERESPONSE) => (
                <RecipeCard
                  key={recipe._id}
                  {...recipe}
                  user={recipe?.user?.email || "Unknown User"}
                />
              ))
            ) : (
              <NoRecipe />
